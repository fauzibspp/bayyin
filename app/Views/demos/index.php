...
<div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <div>
        ...
        <button type="button" class="btn btn-success mr-2" data-toggle="modal" data-target="#modal-create-demos">
            <i class="fas fa-plus"></i> Add New
        </button>
        <button type="button" id="btn-bulk-delete-demos" class="btn btn-danger">
            Bulk Delete
        </button>
    </div>
</div>
...
<div class="card">
    ...
    <div class="card-body">
        <table id="datatable-demos" class="table table-bordered table-striped">
            <thead>
                <tr>
                    <th width="40"><input type="checkbox" id="check-all-demos"></th>
                    <th width="80">ID</th>
                    <th>Name</th>
                    <th>Status</th>
                    <th>Notes</th>
...
<div class="modal fade" id="modal-create-demos" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <form id="form-create-demos">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div id="create-errors-demos" class="alert alert-danger d-none"></div>
...
<div class="modal fade" id="modal-edit-demos" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <form id="form-edit-demos">
            <input type="hidden" name="id" id="edit_id_demos">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    <div id="edit-errors-demos" class="alert alert-danger d-none"></div>
...
<script>
$(function () {
    const table = $('#datatable-demos').DataTable({
        processing: true,
        serverSide: true,
        ajax: '/api/demos/datatable',
        columns: [
            {
                data: null,
                orderable: false,
                ...
                { data: 'is_active' },
            {
                ...
                render: function (data, type, row) {
                    return ''
                        + '<a href="/demos/show?id=' + row.id + '" class="btn btn-info btn-sm mr-1">View</a>'
    ...
    $('#form-create-demos').on('submit', function (e) {
        e.preventDefault();
        clearErrors('#create-errors-demos');
        ...
        $.ajax({
            ...
            success: function () {
                $('#modal-create-demos').modal('hide');
                $('#form-create-demos')[0].reset();
                table.ajax.reload(null, false);
                alert('Record created successfully.');
            },
            error: function (xhr) {
                const response = xhr.responseJSON || {};
                showErrors('#create-errors-demos', response.errors || { general: ['Failed to create record.'] });
            }
        });
    });
    ...
    $(document).on('click', '.btn-edit', function () {
        ...
        clearErrors('#edit-errors-demos');
        $('#edit_id_demos').val(row.id);
        $('#edit_name').val(row.name || '');
        $('#edit_status').val(row.status || '');
        $('#edit_notes').val(row.notes || '');
        $('#edit_is_active').prop('checked', !!parseInt(row.is_active || 0, 10));
        $('#modal-edit-demos').modal('show');
    });

    $('#form-edit-demos').on('submit', function (e) {
        e.preventDefault();
        clearErrors('#edit-errors-demos');
        ...
        $.ajax({
            url: '/api/demos/update',
            type: 'POST',
            contentType: 'application/json',
            data: JSON.stringify(data),
            success: function () {
                $('#modal-edit-demos').modal('hide');
                table.ajax.reload(null, false);
                alert('Record updated successfully.');
            },
            error: function (xhr) {
                const response = xhr.responseJSON || {};
                showErrors('#edit-errors-demos', response.errors || { general: ['Failed to update record.'] });
            }
        });
    });

    $('#check-all-demos').on('change', function () {
    ...
    $('#btn-bulk-delete-demos').on('click', function () {
